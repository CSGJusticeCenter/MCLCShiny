...
df_donutchart <- 
  ...
  filter(metric == "Technical" | metric == "New Offense") %>% select(-year)
...
donut_plot <- ggplot(df_donutchart, aes(y = total, fill = metric)) +
  ...
  annotate(
    ...
    x = 0,
    y = 0,
    label = df_donutchart[["percentage_label"]][df_donutchart[["metric"]] == "Technical"],
    size = 20,
    color = "#E18731"
  ) +
  scale_fill_manual(values = c(`New Offense` = "#DAEAF2", Technical = "#E18731")) +
  coord_polar(theta = "y") +
  theme_void()
