...
server <- function(input, output, session) {
  ...
  alt_text <- reactive({
    paste0("This map captures the changes in ", input$data_map, " ", input$adm_or_pop_map, " from ", input$year_map, ".")
  })
  ...
  foundational_map <- reactive({
    ...
    # highchart hex map
    highchart(list(accessibility = list(
      enabled = TRUE,
      ...
      # title
      hc_title(text = paste0("Change in ", input$data_map, " ", input$adm_or_pop_map, " from ", input$year_map),
               align = "left",
      ...
      hc_legend(align = "right", verticalAlign = "bottom", layout = "vertical") %>% # cant figure out how to add percent labels
      hc_xAxis(
        title = "",
        labels = list(y = 25)
      ) %>%
